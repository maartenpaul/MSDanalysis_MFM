...
p1 <- ggplot(data = ldply(msd_fit_all),aes(x=D,y=..density..,color=inMask_gt1,fill=inMask)) +
...
p1+theme_Publication(base_size=25)

save(p1,file = file.path(directory,"all_histograms.pdf"))

ggplot(data = msd_fit_all$`WT MMC`,aes(x=D,y=..density..,color=inMask_gt2,fill=inMask_gt2)) +
  geom_density(position="identity",alpha=0.5,adjust = 1) +  geom_histogram(position="identity",alpha=0.5)+
