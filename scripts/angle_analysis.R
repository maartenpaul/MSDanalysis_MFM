...
for (k in 1:length(segments_all)){
  ...
  ggplot(inputdata,aes(x = angle,fill=as.character(inMask)))+
   ...
   # scale_x_continuous(breaks=seq(0, 350, 45))+
    geom_histogram()+
   # geom_bar(stat='identity',width=16) +
    #ylim(c(0,.01))+
  ...
  plot_data %>%
    ggplot(aes(x = angle,color=condition))+
    geom_histogram(fill="transparent",breaks=seq(0,360,20),aes(y=..density..),position="identity", lwd=0.8)+
    geom_density(fill="transparent",aes(y=..density..),position="identity", lwd=0.5)+
  ...
  for (i in 1:5){
    ...
    if(i==1){
      ...
      ggplot(angles,aes(x = mids,y=density))+
        geom_hline(yintercept = seq(0, 0.01, by = 0.002), colour = "black", size = 0.2) +
        geom_vline(xintercept = seq(0, 360-1, by = 45), colour = "black", size = 0.2) +
        scale_x_continuous(breaks=seq(0, 350, 45))+
        geom_bar(stat='identity',width=16) +ylim(c(0,.01))+
